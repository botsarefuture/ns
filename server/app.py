...
def get_jobs():
    job = list(
        db.jobs_collection.find({"done": False}, sort=[("priority", pymongo.ASCENDING)])
    )
...
def insert_or_update_ip(ip_address):
    ...
    if existing_record is None:
        hi_collection.insert_one(
            {"ip": ip_address, "last_hi": datetime.now(), "his": [datetime.now()]}
        )
        ...
        db.jobs_collection.insert_one(
            {
                ...
                "job_type": job_type,
                "done": False,
                "priority": functions.get_highest_priority() + 1,
            }
        )
...
def get_clients_count():
    ...
    counts_over_time = list(
        db.pings_collection.aggregate(
            [
    ...
    return jsonify(
        {"clients_count": total_count, "clients_counts_over_time": counts_over_time}
    )
